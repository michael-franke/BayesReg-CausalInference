...
Finally, Section~\ref{sec:programming} explains how we can obtain estimates of causal effects with quantified uncertainty from Bayesian regression models.
...
Section~\ref{sec:conceptual-approach} first explains \textit{what} it is that we are doing (and why it works).
Then, Section~\ref{sec:implementation} shows how to implement this approach in R using the \texttt{brms} package.

\subsection{Conceptual approach}
\label{sec:conceptual-approach}

Based on the previous considerations, we can compute the causal effect of interest as:
\begin{align*}
...
(We would get the same MLE values if had observed that same proportions for a million times more data points.)
...
\begin{enumerate}[1.]
  ...
  \marginnote{We can obtain a prediction of central tendency by sampling many concrete values of the response variable, but that is less efficient than sampling a value of central tendency directly.}

  \item In a Bayesian regression model all of these predictions are probabilistic, including our uncertainty about these values (since we estimate them from limited data).
...
We use logistic regression with the appropriate dependent and independent variables to estimate to get at the relevant conditional or unconditional probabilities.
...
We start by fitting an intercept-only regression model which allows us to estimate the distribution of \textsc{gender}, $P\left( G=g \right)$.
...
To get samples from the posterior predictive distributions of these fitted models, we use the function \ri{epred\_draws()} from the \textit{tidybayes} package.
\marginnote{Notice that the variance in these samples is what quantifies our (modeller's) uncertainty about the value $P(G=1)$.}
To obtain obtain samples of $P(G=1)$, we pass the intercept value of 1.
The sampling result is a probability value between 0 and 1, which we can interpret as the estimated fraction of men in the sample.
...
\vspace{-0.5cm}
The results of this computation are summarize with the \ri{summarize_posterior} function:
...
# A tibble: 3 × 4
  ...
  <fct>            <dbl>  <dbl>    <dbl>
1 take drug       0.790  0.832     0.868
2 refuse drug     0.722  0.778     0.830
3 causal effect  -0.0117 0.0536    0.121
\end{rc}
\end{minipage}

Similar to the MLE approach, we obtain a causal effect estimate of about 5\%, i.e., patients are predicted to have a 5\% higher change of recovery when taking the drug.
However, we can see that the CI includes 0, which suggests that we do not have enough evidence to rule out that the drug had no effect.
...
\vspace{-0.5cm}
As we do not have to do any further calculations to estimate the causal effect in scenario 2, all we need to do is extract the relevant posterior predictive samples, which we can here do by extracting the samples of the linear predictor values and feeding them into a logistic function:
